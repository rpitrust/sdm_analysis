...
#Import individual diffs csv, drops, and add to dataframe
  Indiv.diffs.2.10.15 <- read.csv("M:/R/T1_analysis/Indiv diffs 2.10.15.csv")
  
  Indiv.diffs.2.10.15$UniqueID<-as.numeric(Indiv.diffs.2.10.15$UniqueID)
  #Sort individual diffs by UniqueID 
  Indiv.diffs.Sorted<-arrange(Indiv.diffs.2.10.15, UniqueID)
  
  #Individual diffs UniqueIDs
  #InDiffUniqueID<-unique(as.numeric(Indiv.diffs.Sorted$UniqueID)) 
  #unique(CSVDataFrame$UniqueID)  
  
  #This had better be TRUE; 
  unique(CSVDataFrame$UniqueID)==Indiv.diffs.Sorted$UniqueID
  
  #Only keep UniqueID(s) from individual diffs in the big dataframe: Drops 79 and 131?  
  #CSVDataFrame_drops<-CSVDataFrame[CSVDataFrame$UniqueID %in% InDiffUniqueID,]
      #Check: This had better be TRUE!
      #unique(CSVDataFrame_drops$UniqueID)==(InDiffUniqueID)
  
  #Replicate individual diff measures for each Ss 123 times, match to UniqueID
  Indiv.diffs.Long<-rep(Indiv.diffs.Sorted, each = 123)
      #Check again because I'm a little paranoid
      #unique(CSVDataFrame_drops$UniqueID)==(unique(Indiv.diffs.Long$UniqueID)
  
  #Sort long data by UniqueID 
  Indiv.diffs.Long.Sorted<-arrange(Indiv.diffs.Long, UniqueID)
  
  #Bind individual diffs to 
    #FullData<-cbind(CSVDataFrame_drops )
  
#Drop misspelled final answers, fin_acc = 9, but keep fin_acc = 0 or 1
  SDM_cleaned<-subset(CSVDataFrame, fin_acc!=9)
